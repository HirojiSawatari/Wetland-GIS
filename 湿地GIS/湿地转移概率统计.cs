...
namespace 湿地GIS
{
    public partial class 湿地转移概率统计 : DevComponents.DotNetBar.Metro.MetroForm
    {
        ...
        {
            st = comboBox2.Text.ToString().Trim();
            st1 = comboBox1.Text.ToString().Trim();
            OleDbConnection my_conn;
            OleDbDataAdapter my_adapter;
            DataSet my_ds;
            C_conn c1 = new C_conn();
            my_conn = c1.get_conn();
            string selectstr = "select 类型,建设用地,旱地,林地草地,河流,水库坑塘,水田,沼泽,滩涂,盐田及海水养殖场 from biao2 where 类型 = '" + st1 + "' and 行政区 = '" + st + "'";
            my_adapter = new OleDbDataAdapter(selectstr, my_conn);
            my_ds = new DataSet();
            my_adapter.Fill(my_ds, "biao2");
            dataGridView1.DataSource = my_ds.Tables["biao2"];
            my_conn.Close();
